/**
 * Investigation into storing phrases in Google Docs spreadsheet.
 * There is support for using the sheet offline, however this seems to get upset when on
 * the internal network.  The proxy doesn't allow connection to sheets for some stupid reason.
 * There may be a workaround for this. Regardless, holding the data in a sheet is more
 * convenient to edit, since it's more compact.
 * some stackoverflow refs:
 ...
 * Suggests that the file-level access can be done using the Google Drive Android API
 * which is what I'm currently doing (I think).
 * https://developers.google.com/drive/android/intro
 *
 * https://developers.google.com/drive/android/get-started
 * Note the setting up of the OAuth2.0 client
 * https://developers.google.com/drive/android/files
 * Has example code to open read file.
 * https://developers.google.com/drive/android/metadata
 * The metadata may show how to determine if the file is text or a sheet.
 *
 * https://developers.google.com/sheets/api/quickstart/android
 * This is the sheets quickstart, maybe this is needed to access sheets files.
 * https://developers.google.com/resources/api-libraries/documentation/sheets/v4/java/latest/
 * Not sure, but this may be the sheets api docs
 *
 *
 * Credentials
...
public class SheetsActivity
    ...
    private class MakeRequestTask extends AsyncTask<Void, Void, List<String>> {
         ...
         * @throws IOException
         */
        private List<String> getDataFromOdile() throws IOException {
            String spreadsheetId = "1l_uoIHCQeztpWnsjP6reufTy-oqXEJP2SrTjhX0-_o0";
            String range = "!A2:C";
            List<String> results = new ArrayList<String>();
            ValueRange response = this.mService.spreadsheets().values()
                                               .get(spreadsheetId, range)
